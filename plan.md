...
- [x] **ID-25 / CS-07 スパイク → NO-GO でクローズ**（2026-09-02）: `experiments/cs07_veg_price/`。
  産地(諏訪)の週次日照平年比 → 2〜4週後の東京レタス卸売価格。**ラグ3週 ρ=−0.16, p=0.064**（事前基準 p<0.05 未達）、効果量 r²≈3%。
  符号・時間順序・季節依存は仮説と整合するが統計的裏づけなし。データ拡張の費用対効果が見合わず打ち切り。詳細と考察は当該 README。
  副産物: 気象庁 etrn ＋ ベジ探 sch7.do の**完全自動取得**コード（鍵不要・手動DL不要）を資産として保持。
