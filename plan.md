...
- [x] **ID-25 / CS-07 スパイク → NO-GO でクローズ（2品目検証）**（2026-09-02）: `experiments/cs07_veg_price/`。
  主産地の週次日照平年比 → 2〜4週後の東京卸売価格。**レタス(諏訪) ラグ3週 ρ=−0.16 p=0.064／ほうれんそう(前橋) ラグ1週 ρ=+0.18 p=0.036（符号が逆・多重比較で非有意）**。いずれも r²≈3% の弱域。
  レタスは仮説どおり日照不足→高値、ほうれんそうは日照過多→速く高値（暑さ・抽苔に弱い生理と整合）だが CS-07 の枠組み外。データ拡張の費用対効果が見合わず打ち切り。考察は当該 README。
  副産物: 気象庁 etrn ＋ ベジ探 sch7.do の**完全自動取得**コード（鍵不要・`ACTIVE_CROP` で品目切替）を資産として保持。
