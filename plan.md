...
- [x] **ID-25 / CS-07 スパイク → 一部GO**（2026-09-02）: `experiments/cs07_veg_price/`。
  ...
  ③ その1本を**単一仮説として事前登録**し、ベジ探 月別で期間を15年（2011〜2026, n=186）に拡張、自己相関に頑健な循環置換検定で検証:
     **前橋の真夏日数の月次季節偏差 → 1〜2か月後の東京ほうれんそう卸売価格残差、ρ=+0.34（暖候期6〜10月のみ +0.43, r²≈0.18）、循環置換 p<0.001。→ H1 支持。**
  週次2.7年では同符号だが検出力不足で非有意。次: 交絡統制つきの分布ラグ回帰（[docs/ml_models.md](docs/ml_models.md)）。
