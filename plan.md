...
- [x] **ID-25 / CS-07 スパイク → 一部GO**（2026-09-02）: `experiments/cs07_veg_price/`。
  ...
  ⑤ 産地合成（群馬/茨城/栃木/埼玉/千葉/岩手/岐阜 の7官署真夏日数を東京入荷ウェイトで加重）→ 価格 L1 β=+0.0103（t=+3.05）で前橋1点とほぼ同じ。
     入荷量の回帰: 合成真夏日数偏差 → log入荷量 L0 β=−0.011(t=−4.6), L1 β=−0.010(t=−3.2)、真夏日+10日で入荷量−15%。**価格上昇は供給減が経路**（量−15%↔価格+10%、需要弾力性≈−0.7で整合）。
     人為的要因: 効果が同月でなく1か月前に集中する時間構造は「暑い時期の作付けを減らす／遅らせる」判断＋発芽/生育不良と整合。ただし直接被害と作付け判断は月次では分離不能。暑夏に涼しい産地へ振り替える適応は確認できず（冷涼産地シェアは2011年25%→2025年7%と構造的減少）。
  → **ID-25 の中核仮説（産地の気象ストレスが1か月先行して価格を動かす）を、ほうれんそう×高温について 相関→頑健性→分布ラグ回帰→供給チャネル まで確認。**
  次: 日次で2023年以前を e-Stat 日別調査から補完、熱帯夜/WBGT で効果量比較、デモ化。
  副産物: 気象庁 etrn（p1+a3・2011〜・12官署）＋ ベジ探 sch7.do（日別／月別／産地別数量）の**完全自動取得**コード＋分布ラグ回帰スクリプト2本。
