...
- [~] **ID-25 / CS-07 スパイク（2品目・気象拡張）**（2026-09-02）: `experiments/cs07_veg_price/`。
  ① 日照のみ: レタス(諏訪) lag3 ρ=−0.16 p=0.064／ほうれんそう(前橋) lag1 ρ=+0.18 → いずれも NO-GO。
  ② 気象を 日照・全天日射・降水・高温日数 に拡張: **ほうれんそう×真夏日数(≥30℃) lag3週 ρ=+0.32 (p=0.008, r²≈0.10)** が最有力。
     レタス/長野は日照 lag3-4 ρ=−0.19 (p=0.024) で当初GO条件を名目達＋日射・降水も同方向。ただし 5特徴×9ラグ=45セルの多重比較で厳密には非有意、期間も140週のみ。
  → **保留**。次: 期間を2018年以前へ延長し「ほうれんそう×真夏日数」を単一仮説で事前登録して再検証。
  副産物: 気象庁 etrn（p1+a3 の複数要素）＋ ベジ探 sch7.do の**完全自動取得**コード（`ACTIVE_CROP` で品目切替）を保持。
