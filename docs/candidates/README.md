...
> **データルール（[../../CLAUDE.md](../../CLAUDE.md)）**: 気象は気象庁のみ（過去 = obsdl、実況・予報 = bosai、天気図・衛星 = 気象庁画像）。相手側は公開データのみ。個人データ不可。GRIB 不使用。
> → ID-17 の波浪は気象庁の公開データで1年分そろえられるか要確認（GRIB の GPV は不可）。難しければ「波」を外し天気・風・潮位・UV でスコアを組む。
...
|---|---|---|---|
| データ入手 | 気象庁の天気図・衛星画像のみ（◎） | 気象庁 obsdl（天気・風・日照）＋気象庁 潮位。波浪は気象庁の公開範囲を確認（△） | 気象庁 obsdl の気圧のみ（◎） |
