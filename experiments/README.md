...
| [cs32_chart_vlm/](cs32_chart_vlm/) | [ID-32](../docs/candidates/c32_weather_chart_vlm.md) | 天気図・衛星赤外をVLMに読ませ、実況・予報でガードレール検証（1事例） | **一部GO（2026-09-25）**。気象庁の天気図/衛星/アメダス取得〜検証の配管は成立、VLM役が高低気圧・前線・台風を読み予報概況と矛盾なし。自己テスト3/3（初回2/3→季節整合を追加）。事例1件・非盲検で精度は未検証 |
